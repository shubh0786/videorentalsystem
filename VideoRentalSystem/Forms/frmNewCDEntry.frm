...
Begin VB.Form frmNewCDEntry 
   ...
   Begin VB.PictureBox Picture1 
      ...
      End
   End
   Begin XtremeSuiteControls.PushButton bttnNewEntry 
      Height          =   375
      Left            =   600
      TabIndex        =   4
      Top             =   7080
      ...
      EndProperty
      Appearance      =   6
      Picture         =   "frmNewCDEntry.frx":0000
   End
   Begin XtremeSuiteControls.PushButton bttnSave 
      ...
      EndProperty
      Appearance      =   6
      Picture         =   "frmNewCDEntry.frx":039A
   End
   Begin XtremeSuiteControls.PushButton bttnCancel 
      ...
      EndProperty
      Appearance      =   6
      Picture         =   "frmNewCDEntry.frx":0734
   End
   ...
   Begin MSComDlg.CommonDialog CommonDialog1 
      ...
      _Version        =   393216
   End
   Begin XtremeSuiteControls.PushButton Update 
      Height          =   375
      Left            =   7440
      TabIndex        =   35
      Top             =   7080
      ...
      EndProperty
      Appearance      =   6
      Picture         =   "frmNewCDEntry.frx":0ACE
   End
   ...
   End
End
Attribute VB_Name = "frmNewCDEntry"
...
Else
    ...
    Me.bttnUpdate.Enabled = True
    Me.bttnNewEntry.Enabled = True
End If
End Sub
...
Private Sub bttnNewEntry_Click()
Call unlockF
    Me.txtDVDNo.Enabled = False
    Me.txtTitle.SetFocus
    Me.bttnCancel.Caption = "Cancel"
    Me.bttnUpdate.Enabled = False
    'Me.bttnSave.Enabled = True
    nwEntry = True
    Call newEntry
End Sub
...
Set rs = New ADODB.Recordset
    With rs
               .Open "Select * from tbl_CDList where CD_ID like '" & txtDVDNo.Text & "' and Status like 'AVAILABLE'", cn, 1, 2
...
x = Me.lstID.ListIndex
Set rs = New ADODB.Recordset
With rs
    .Open "Select * from tbl_CDList where CD_ID like '" & Me.lstID.List(x) & "' and Status like 'AVAILABLE'", cn, 1, 2
    
    Me.txtDVDNo.Text = Me.lstID.List(x)
    ...
    ' txtTitle.Text = !Title
'         txtGenre.Text = !Ggenre
'          txtDirector.Text = !director
'       txtActor.Text = !actor
'        txtYearRealese.Text = !YearRelease
'        txtCDType.Text = !CDType
...
'    .Close
  ...
  '  .Open "Select * from tbl_CDList where CD_ID like '" & search & "' and Status like 'AVAILABLE'", cn, 1, 2
'    If .EOF = True Then
...
'On Error GoTo err
'    Dim byteData() As Byte
'If nwEntry = True And discUpdate = False Then
'
'    Open CommonDialog1.FileName For Binary As #1
'    ReDim byteData(FileLen(CommonDialog1.FileName))
'    Get #1, , byteData
...
'            Set rs = Nothing
'
'   If discUpdate = True And nwEntry = False Then
'
'    bttnNewEntry.Enabled = False
'  '   Dim byteData() As Byte
'
'    If CommonDialog1.FileName <> "" Then
'    Open CommonDialog1.FileName For Binary As #1
...
'          If CommonDialog1.FileName <> "" Then !pic.AppendChunk byteData
'        .Update
'        wDone = "Edited DISC " & "/" & "DISC ID" & " - " & Me.txtDVDNo.Text
'        Call auditT
'
'        If MsgBox("Record Successfully Updated.", vbInformation + vbOKOnly) = vbOK Then Call clearF
'
'         Me.bttnSave.Enabled = False
'        Me.bttnCancel.Caption = "Close"
'        Call cboItems
...
'search = InputBox("Enter Disc ID to edit:")
'    Set rs = New ADODB.Recordset
'    With rs
'     .Open "Select * from tbl_CDList where CD_ID like '" & search & "' and Status like 'AVAILABLE'", cn, 1, 2
'    If .EOF = True Then
