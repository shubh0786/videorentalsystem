...
Begin VB.Form frmCustomerReg 
   ...
   Begin XtremeSuiteControls.TabControl tabMain 
      ...
      Begin XtremeSuiteControls.RadioButton rBttn3 
         ...
         _ExtentY        =   450
         _StockProps     =   79
         Caption         =   "Registered mail not older than 2 months"
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "MS Sans Serif"
            Size            =   9.75
            Charset         =   0
...
Set rs = New ADODB.Recordset
    With rs
        .Open "Select * from tbl_Members", cn, 1, 2
        ...
        wDone = "Add New Member" & "/" & "Member ID" & " - " & Me.txtMemberID.Text
            Call auditT
            
        If MsgBox("Record Successfully Added.", vbInformation + vbOKOnly) = vbOK Then Call Clear
            tabMain.SelectedItem = 0
        Call getCount
